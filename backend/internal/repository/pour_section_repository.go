...
func (repository *pourSectionRepository) List(ctx context.Context, query dto.PourSectionQuery) ([]model.PourSection, int64, error) {
	database := repository.db.Model(&model.PourSection{})
...
func (repository *pourSectionRepository) GetByID(ctx context.Context, id uint) (model.PourSection, error) {
	...
	if err := repository.db.Preload("MixDesign").First(&section, id).Error; err != nil {
...
func (repository *pourSectionRepository) Summary(ctx context.Context, sectionID uint) (model.PourSectionSummary, error) {
	...
	if err := repository.db.Model(&model.TemperatureSeries{}).Where("pour_section_id = ?", sectionID).Count(&summary.TemperatureSeriesCount).Error; err != nil {
		return summary, fmt.Errorf("count section temperature series: %w", err)
	}
	if err := repository.db.Model(&model.StrengthForecast{}).Where("pour_section_id = ?", sectionID).Count(&summary.ForecastCount).Error; err != nil {
		return summary, fmt.Errorf("count section forecasts: %w", err)
	}
	var latest model.StrengthForecast
	err := repository.db.Where("pour_section_id = ? AND forecast_state IN ?", sectionID, []string{"completed", "reviewed", "confirmed"}).Order("calculated_at DESC").First(&latest).Error
